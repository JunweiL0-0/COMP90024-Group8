...
def main():
    ...
    election_words = util.get_words('election_words.json')
    ...
    for line in geo_twitter_json_file:
        ...
        if place_code is not None:  # if place code is not none
            for word in data['doc']['data']['text'].split():  # check if covid or election
                if word in covid_words:  # if covid word
                    is_covid = True
        ...
        if len(json_to_write["docs"]) > 1000:
            write_file = open(f"geo-twitter-{counter}.json", 'w')   # open file
    ...
    if len(json_to_write["docs"]) > 0:
        write_file = open(f"geo-twitter-{counter}.json", 'w')  # open file and write if there are any data left
        write_file.write(json.dumps(json_to_write))  # write to file
        write_file.close()  # close file
